...
const handleFile = async (hookObject: HookObject<any>) => {
	...
	const schema = hookObject.context.wibe.config.schema.classes.find(
	...
	await Promise.all(
		Object.keys(newData).map(async (keyName) => {
			...
			if (hookObject.context.wibe.config.file?.adapter !== undefined)
				hookObject.upsertNewData(
					keyName,
					await hookObject.context.wibe.config.file.adapter(
						newData[keyName],
					),
				)
		}),
