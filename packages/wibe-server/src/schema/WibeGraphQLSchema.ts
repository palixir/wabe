    ...
    const graphqlFields = fieldsOfObjectKeys.reduce(
      (acc, fieldName) => {
        ...
        if (currentField.type === "Object") {
          ...
          if (!object)
            throw new Error(`Failed to create ${currentField.object.name}`);
  ...
  createDefaultMutationsSchema({
    ...
    const mutations: Record<string, GraphQLFieldConfig<any, any, any>> = {
      ...
      [`createMany${className}`]: {
        ...
        resolve: (root, args, ctx, info) =>
          mutationToCreateMultipleObjects(root, args, ctx, info, className),
      },
      [`updateOne${className}`]: {
        type: new GraphQLNonNull(object),
      ...
      [`updateMany${className}`]: {
        ...
        resolve: (root, args, ctx, info) =>
          mutationToUpdateMultipleObjects(root, args, ctx, info, className),
      },
      [`deleteOne${className}`]: {
        type: new GraphQLNonNull(object),
        args: {
      ...
      [`deleteMany${className}`]: {
        ...
        resolve: (root, args, ctx, info) =>
          mutationToDeleteMultipleObjects(root, args, ctx, info, className),
      },
    };
