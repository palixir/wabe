import { describe, it, expect, spyOn, beforeAll, afterAll, afterEach } from 'bun:test';
import { setupInMemoryPostgres, testPostgresClient } from './index.ts'; 
...
describe('In-Memory PostgreSQL Setup Tests', () => {
    ...
    it('should select records with a WHERE clause', async () => {
        ...
        const result = await inMemoryClient!.query(`SELECT name FROM users WHERE name = 'Jane Doe'`);
    ...
    it('should create a table and verify its existence', async () => {
        ...
        // Create the table
        await inMemoryClient!.query(`CREATE TABLE test_table (id SERIAL PRIMARY KEY, name VARCHAR(255))`);
    ...
    it('should allow concurrent connections to separate in-memory databases', async () => {
        const client1 = await setupInMemoryPostgres();
        const client2 = await setupInMemoryPostgres();
    
        await client1!.query(`CREATE TABLE test (id SERIAL PRIMARY KEY, name VARCHAR(255))`);
        await client2!.query(`CREATE TABLE test (id SERIAL PRIMARY KEY, name VARCHAR(255))`);
    ...
    it('should not persist schema between tests', async () => {
        const inMemoryClient = await setupInMemoryPostgres();
        await inMemoryClient!.query(`CREATE TABLE temp_table (id SERIAL PRIMARY KEY)`);
    
        const checkTable = await inMemoryClient!.query(`SELECT * FROM information_schema.tables WHERE table_name = 'temp_table'`);
        expect(checkTable.rows.length).toBe(1);
    
        await inMemoryClient!.end();
    
        // In the next connection, temp_table should not exist
        const newInMemoryClient = await setupInMemoryPostgres();
        const checkTableAgain = await newInMemoryClient!.query(`SELECT * FROM information_schema.tables WHERE table_name = 'temp_table'`);
    ...
    it('should enforce unique constraints', async () => {
        ...
        await inMemoryClient!.query(`INSERT INTO unique_test (email) VALUES ('unique@example.com')`);
    
        await expect(
            inMemoryClient!.query(`INSERT INTO unique_test (email) VALUES ('unique@example.com')`)
        ).rejects.toThrow();
    ...
    it('should persist data across queries within the same test', async () => {
        ...
        await inMemoryClient!.query(`CREATE TABLE persistence_test (id SERIAL PRIMARY KEY, name VARCHAR(255))`);
        await inMemoryClient!.query(`INSERT INTO persistence_test (name) VALUES ('Persistent Entry')`);
    
        const result = await inMemoryClient!.query(`SELECT * FROM persistence_test WHERE name = 'Persistent Entry'`);
    ...
    it('should respect case sensitivity in column names', async () => {
        ...
        await inMemoryClient!.query(`INSERT INTO case_test ("CaseSensitiveName") VALUES ('Test Value')`);
        const result = await inMemoryClient!.query(`SELECT "CaseSensitiveName" FROM case_test`);
        expect(result.rows[0].CaseSensitiveName).toBe('Test Value');
    
        // This query should fail because it uses the wrong case
        await expect(inMemoryClient!.query(`SELECT casesensitivename FROM case_test`)).rejects.toThrow();
    
        await inMemoryClient!.end();
    });
    it('should handle large amounts of data', async () => {
        const inMemoryClient = await setupInMemoryPostgres();
        await inMemoryClient!.query(`CREATE TABLE large_data_test (id SERIAL PRIMARY KEY, value VARCHAR(255))`);
    
        const insertPromises = [];
        for (let i = 0; i < 1000; i++) {
            insertPromises.push(
                inMemoryClient!.query(`INSERT INTO large_data_test (value) VALUES ($1)`, [`Value ${i}`])
            );
        }
        await Promise.all(insertPromises);
    
        const countResult = await inMemoryClient!.query(`SELECT COUNT(*) FROM large_data_test`);
    ...
    it('should insert and retrieve multiple rows', async () => {
        const inMemoryClient = await setupInMemoryPostgres();
        await inMemoryClient!.query(`CREATE TABLE items (id SERIAL PRIMARY KEY, name VARCHAR(255))`);
    
        // Insert multiple rows
        await inMemoryClient!.query(`INSERT INTO items (name) VALUES ('Item 1'), ('Item 2'), ('Item 3')`);
    ...
    it('should delete data from the table', async () => {
        const inMemoryClient = await setupInMemoryPostgres();
        await inMemoryClient!.query(`CREATE TABLE deletable (id SERIAL PRIMARY KEY, name VARCHAR(255))`);
    
        // Insert a row
        await inMemoryClient!.query(`INSERT INTO deletable (name) VALUES ('Deletable Item')`);
    
        // Delete the row
        await inMemoryClient!.query(`DELETE FROM deletable WHERE name = 'Deletable Item'`);
    
        // Verify that no rows remain
        const result = await inMemoryClient!.query(`SELECT * FROM deletable WHERE name = 'Deletable Item'`);
    ...
    it('should update data in the table', async () => {
        const inMemoryClient = await setupInMemoryPostgres();
        await inMemoryClient!.query(`CREATE TABLE updatable (id SERIAL PRIMARY KEY, name VARCHAR(255))`);
    
        // Insert a row
        await inMemoryClient!.query(`INSERT INTO updatable (name) VALUES ('Original Name')`);
    
        // Update the row
        await inMemoryClient!.query(`UPDATE updatable SET name = 'Updated Name' WHERE name = 'Original Name'`);
    
        // Verify the update
        const result = await inMemoryClient!.query(`SELECT name FROM updatable WHERE name = 'Updated Name'`);
        expect(result.rows.length).toBe(1);
    ...
    it('should rollback changes in a nested transaction', async () => {
        const inMemoryClient = await setupInMemoryPostgres();
        await inMemoryClient!.query(`CREATE TABLE nested_transaction_test (id SERIAL PRIMARY KEY, name VARCHAR(255))`);
    
        await inMemoryClient!.query('BEGIN');
        try {
            // Insert a row in outer transaction
            await inMemoryClient!.query(`INSERT INTO nested_transaction_test (name) VALUES ('Outer Transaction')`);
    
            // Start an inner transaction (sub-transaction)
            await inMemoryClient!.query('SAVEPOINT sp1');
            try {
                await inMemoryClient!.query(`INSERT INTO nested_transaction_test (name) VALUES ('Inner Transaction')`);
        ...
        const result = await inMemoryClient!.query(`SELECT name FROM nested_transaction_test`);
        expect(result.rows.length).toBe(1);
    ...
    it('should insert and retrieve a single row', async () => {
        ...
        // Create the table
        await inMemoryClient!.query(`CREATE TABLE users (id SERIAL PRIMARY KEY, name VARCHAR(255))`);
    
        // Insert a single row
        await inMemoryClient!.query(`INSERT INTO users (name) VALUES ('John Doe')`);
    
        // Retrieve the inserted row
        const result = await inMemoryClient!.query(`SELECT name FROM users WHERE name = 'John Doe'`);
    ...
    it('should insert a row, update it, and verify the update', async () => {
        ...
        // Update the row
        await inMemoryClient!.query(`UPDATE users SET name = 'Alice Smith' WHERE name = 'Alice'`);
    
        // Retrieve the updated row
        const result = await inMemoryClient!.query(`SELECT name FROM users WHERE name = 'Alice Smith'`);
    ...
    it('should enforce unique constraint on a column', async () => {
        ...
        await inMemoryClient!.query(`CREATE TABLE users (id SERIAL PRIMARY KEY, name VARCHAR(255), email VARCHAR(255) UNIQUE)`);
    
        // Insert a row
        await inMemoryClient!.query(`INSERT INTO users (name, email) VALUES ('Charlie', 'charlie@example.com')`);
    
        // Try inserting another row with the same email (should fail due to the unique constraint)
        let errorOccurred = false;
        try {
            await inMemoryClient!.query(`INSERT INTO users (name, email) VALUES ('Charlie Duplicate', 'charlie@example.com')`);
        } catch (error) {
    ...
    it('should count the number of rows in a table', async () => {
        ...
        // Insert some rows
        await inMemoryClient!.query(`INSERT INTO users (name) VALUES ('John'), ('Jane'), ('Alice')`);
    ...
    it('should handle NULL values in columns correctly', async () => {
        ...
        // Create the table
        await inMemoryClient!.query(`CREATE TABLE users (id SERIAL PRIMARY KEY, name VARCHAR(255), email VARCHAR(255))`);
    
        // Insert a row with a NULL email
        await inMemoryClient!.query(`INSERT INTO users (name, email) VALUES ('David', NULL)`);
    
        // Retrieve the row where email is NULL
        const result = await inMemoryClient!.query(`SELECT email FROM users WHERE name = 'David'`);
    ...
    it('should join two tables and retrieve related data', async () => {
        ...
        // Insert users
        await inMemoryClient!.query(`INSERT INTO users (name) VALUES ('John'), ('Jane')`);
    
        // Insert orders for the users
        await inMemoryClient!.query(`INSERT INTO orders (user_id, amount) VALUES (1, 100.50), (2, 250.75)`);
    ...
    it('should group by a column and aggregate data', async () => {
        ...
        expect(result.rows[2].total_spent).toBe(150.00); // Sum of orders for user 3
    ...
    it('should left join and return rows even with no matching data', async () => {
        ...
        await inMemoryClient!.query(`INSERT INTO orders (user_id, amount) VALUES (1, 100.50)`);
    ...
    it('should inner join with multiple conditions and return correct results', async () => {
        ...
        // Insert users
        await inMemoryClient!.query(`INSERT INTO users (name, city) VALUES ('John', 'New York'), ('Jane', 'Boston'), ('Alice', 'Chicago')`);
