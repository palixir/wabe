...
import { WabeInMemoryPostgres, WabePostgresClient } from './index.ts'
import tcpPortUsed from 'tcp-port-used'
import { Client as PgClient } from 'pg'
import { type nil } from 'pg-mem'

let client: PgClient | nil // Global variable to track the client
...
describe('Wabe In-Memory PostgreSQL Setup Tests (WabeInMemoryPostgres)', () => {
  ...
  it('should create, insert, and retrieve a row in the wabe table', async () => {
    const memoryDb = await WabeInMemoryPostgres()
    ...
    const insertQuery = `
        ...
        RETURNING *;
    `
    const insertResult = await memoryDb?.query(insertQuery, [
      nextId++,
      'Test Project',
      'test@example.com',
    ])
    ...
    // Use the same memoryDb client for the select query
    const selectQuery = 'SELECT * FROM wabe WHERE email = $1;'
    const selectResult = await memoryDb?.query(selectQuery, [
      'test@example.com',
    ])
